...
class indexer:
    ...
    def youtube(self, url, action):
        try:
            ...
            if 'PlaylistTuner' in action:
                self.list = cache.get(
                    youtube.youtube(
                        key=key).playlist, 1, url)
            elif 'Playlist' in action:
                self.list = cache.get(
                    youtube.youtube(
                        key=key).playlist, 1, url, True)
            elif 'ChannelTuner' in action:
                self.list = cache.get(youtube.youtube(key=key).videos, 1, url)
            elif 'Channel' in action:
                self.list = cache.get(
                    youtube.youtube(
                        key=key).videos, 1, url, True)

            if 'Tuner' in action:
                for i in self.list:
                    i.update({'name': i['title'],
                              'poster': i['image'],
                              'action': 'plugin',
                              'folder': False})
                if 'Tuner2' in action:
                    self.list = sorted(
                        self.list, key=lambda x: random.random())
                self.addDirectory(self.list, queue=True)
            else:
                for i in self.list:
                    i.update({'name': i['title'],
    ...
    def tvtuner(self, url):
        try:
            ...
            url, imdb, tvdb, tvshowtitle, year, thumbnail, fanart = re.findall(
                '<url>(.+?)</url>', url)[0], re.findall(
                '<imdb>(.+?)</imdb>', url)[0], re.findall(
                '<tvdb>(.+?)</tvdb>', url)[0], re.findall(
                '<tvshowtitle>(.+?)</tvshowtitle>', url)[0], re.findall(
                    '<year>(.+?)</year>', url)[0], re.findall(
                        '<thumbnail>(.+?)</thumbnail>', url)[0], re.findall(
                            '<fanart>(.+?)</fanart>', url)[0]

            tvm = client.request(
                'http://api.tvmaze.com/lookup/shows?thetvdb=%s' %
                tvdb)
            if tvm is None:
                tvm = client.request(
                    'http://api.tvmaze.com/lookup/shows?imdb=%s' %
                    imdb)
            tvm = 'http://api.tvmaze.com/shows/%s/episodes' % str(
                json.loads(tvm).get('id'))
            items = json.loads(client.request(tvm))
            items = [(str(i.get('season')), str(i.get('number')), i.get(
                'name').strip(), i.get('airdate')) for i in items]

            if preset == 'tvtuner':
                choice = random.choice(items)
                items = items[items.index(choice):] + \
                    items[:items.index(choice)]
                items = items[:100]
    ...
    def search(self):
        try:
            ...
            try:
                ...
                for url in query:
                    try:
                        self.list += [{'name': '%s...' % url,
                                       'url': url, 'action': 'addSearch'}]
                    except BaseException:
                        pass
            except BaseException:
                pass
    ...
    def noname_list(self, url, result=None):
        ...
        for item in items:
            try:
                ...
                try:
                    premiered = re.findall(
                        '<premiered>(.+?)</premiered>', meta)[0]
                except BaseException:
                    premiered = '0'
    ...
    def movie_info(self, i):
        try:
            ...
            if 'Error' in item and 'incorrect imdb' in item['Error'].lower():
                return self.meta.append({'imdb': imdb,
                                         'tmdb': '0',
                                         'tvdb': '0',
                                         'lang': self.lang,
                                         'item': {'code': '0'}})
    ...
    def tv_info(self, i):
        try:
            ...
            item = client.request(
                url, output='extended', error=True, timeout='10')

            if item[1] == '404':
                return self.meta.append({'imdb': '0',
                                         'tmdb': '0',
                                         'tvdb': tvdb,
                                         'lang': self.lang,
                                         'item': {'code': '0'}})
    ...
    def addDirectory(self, items, queue=False):
        ...
        try:
            devmode = True if 'testings.xml' in control.listDir(control.dataPath)[
                1] else False
        except BaseException:
            devmode = False
        ...
        for i in items:
            try:
                ...
                if content in ['movies', 'tvshows', 'seasons', 'episodes']:
                    cm.append(
                        (control.lang(30708).encode('utf-8'),
                         'XBMC.Action(Info)'))

                if (folder == False and '|regex=' not in str(i.get('url'))) or (
                ...
                if content == 'movies':
                    ...
                    try:
                        dfile = '%s S%02dE%02d' % (
                            i['tvshowtitle'], int(i['season']), int(i['episode']))
                    except BaseException:
                        dfile = name
                    try:
                        cm.append(
        ...
        try:
            ...
            url = '%s?action=%s&url=%s' % (
                sysaddon, i['nextaction'], urllib.quote_plus(i['next']))
            item = control.item(label=control.lang(30500).encode('utf-8'))
            item.setArt({'addonPoster': addonPoster,
                         'thumb': addonPoster,
                         'poster': addonPoster,
...
class resolver:
    ...
    def get(self, url):
        try:
            items = re.compile(
                '<sublink(?:\s+name=|)(?:\'|\"|)(.*?)(?:\'|\"|)>(.+?)</sublink>').findall(url)
            ...
            select = control.selectDialog(
                [i[0] for i in items], control.infoLabel('listitem.label'))
    ...
    def f4m(self, url, name):
            try:
                ...
                try:
                    proxy_use_chunks = json.loads(
                        params['proxy_for_chunks'][0])
                except BaseException:
                ...
                try:
                    simpleDownloader = json.loads(
                        params['simpledownloader'][0])
                except BaseException:
        ...
        try:
            ...
            try:
                tvdb, tvshowtitle, premiered, season, episode = re.findall(
                    '<tvdb>(.+?)</tvdb>', url)[0], re.findall(
                    '<tvshowtitle>(.+?)</tvshowtitle>', url)[0], re.findall(
                    '<premiered>(.+?)</premiered>', url)[0], re.findall(
                    '<season>(.+?)</season>', url)[0], re.findall(
                    '<episode>(.+?)</episode>', url)[0]
            except BaseException:
...
class player(xbmc.Player):
    ...
    def play(self, url, content=None):
        try:
            ...
            self.getbookmark = True if (
                content == 'movies' or content == 'episodes') else False
    ...
    def onPlayBackStarted(self):
        ...
        if self.getbookmark and not self.offset == '0':
...
class bookmarks:
    def get(self, name, year='0'):
        try:
            ...
            dbcur = dbcon.cursor()
            dbcur.execute(
                "SELECT * FROM bookmark WHERE idFile = '%s'" %
                idFile)
            ...
            try:
                ...
                yes = control.yesnoDialog(label, '', '', str(name), control.lang(
                    32503).encode('utf-8'), control.lang(32501).encode('utf-8'))
