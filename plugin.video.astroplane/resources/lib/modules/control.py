...
def addonThumb():
    ...
    elif theme == '-': return 'DefaultFolder.jpg'
    return addonInfo('icon')
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Superbeast.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Superbeast.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Superbeast.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Superbeast.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.Nightmare.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Nightmare.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
