...
def downloader():
    ...
    def download(): return []
    result = cache.Nameless_download_get(download, 600000000, table='rel_dl')
...
def addDownload(name, url, image, provider=None):
    ...
    from resources.lib.indexers import Namelessstreams
    url = Namelessstreams.resolver().link(url)
    if url == None: return
    ...
    result = cache.Nameless_download_get(download, 0, table='rel_dl')
