...
def handle_db_movies(movie):
    trailer = "plugin://script.interdimensional/?info=playtrailer&dbid=%s" % str(movie['movieid'])
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.interdimensional/?info=interdimensional&dbid=%s' % str(movie['movieid'])
    else:
        path = 'plugin://script.interdimensional/?info=playmovie&dbid=%i' % movie['movieid']
...
#        if movie["art"].get('animatedposter', ""): poster = movie["art"].get('animatedposter', "")
#        else: poster = movie["art"].get('poster', "")
#        if movie["art"].get('animatedfanart', ""): fanart = movie["art"].get('animatedfanart', "")
#        else: fanart = movie["art"].get('fanart', "")
#    else:
...
def handle_db_tvshows(tvshow):
    if SETTING("infodialog_onclick") != "false":
        path = 'plugin://script.interdimensional/?info=extendedtvinfo&dbid=%s' % str(tvshow['tvshowid'])
    else:
        path = 'plugin://script.interdimensional/?info=action&id=ActivateWindow(videos,videodb://tvshows/titles/%s/,return)' % str(tvshow['tvshowid'])
...
def compare_album_with_library(online_list):
    ...
    for online_item in online_list:
        for local_item in local_list:
            ...
            online_item["path"] = 'plugin://script.interdimensional/?info=playalbum&dbid=%i' % album['albumid']
            if album["thumbnail"]:
