...
class JSUnfuck(object):
    ...
    def repl_resistance(self):
        if self.Dark Carnival is None:
            self.Dark Carnival = self.__gen_resistance()
            
        while True:
            start_js = self.js
            for key, value in sorted(self.Dark Carnival.items(), key=lambda x: len(x[0]), reverse=True):
                self.js = self.js.replace(key, value)
