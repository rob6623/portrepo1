...
plugin = Plugin('Toxic Creator')
big_list_view = False

namechange_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/namechange/')
path_to_ivue = xbmc.translatePath('special://profile/addon_data/script.GCguide')
path_to_creator = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator')
...
addons2_filename = xbmc.translatePath('special://profile/addon_data/script.GCguide/addons2.ini')
addons2_ini = cp(addons2_filename)

addons_ini_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/addons_index.ini')
...
@plugin.route('/clear')
def clear():
    storagePath = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/.storage')
    namechange_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/namechange')
    ...
    if sanitycheck('Clear all subs?', 'are you sure?'):
        ...
        if sanitycheck('Toxic Creator','Delete all namechange ini\'s?'):
...
def remove_addon(id):
    nc_id = re.sub('\.','-',id)
    storagePath = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/.storage/%s' % id)
    namechange_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/namechange/%s' % nc_id +'.ini')
    path = "plugin://%s" % id

    if id == 'script.GCguide':
    ...
    if sanitycheck('Clear ' + str(fancy_label), 'Are you sure?'):
        ...
        if os.path.exists(namechange_path):
            if sanitycheck('Toxic Creator','Delete %s \'s namechange ini?' % fancy_label):
...
def updateaddon():
    ...
    if len(sections) < 1:
        message('No addons detected', 'Toxic Creator')
        return
...
def update(id = '',silent = True, ini_label = ''):
    ...
    if ( xbmcgui.getCurrentWindowId() == 10025 ):
        ...
        xbmc.executebuiltin('RunScript(script.GCguide, run)')
