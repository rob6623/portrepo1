...
class tvshows:
    def __init__(self):
        ...
        self.tvdb_key = base64.urlsafe_b64decode('REFFMjMxOEMzMjAwNERFMw==')
        ...
        self.tmdbwartv_link = 'http://api.themoviedb.org/3/list/?api_key=%s' % (self.tmdb_key)
        ...
        self.tmdbldtv_link = 'http://api.themoviedb.org/3/list/35888?api_key=%s' % (self.tmdb_key)
    ...
    def years(self):
        ...
        for i in range(int(year)-0, int(year)-50, -1): self.list.append({'name': str(i), 'url': self.year_link % ('%s', str(i)), 'image': 'icon.png', 'action': 'movies'})
    ...
    # def genres(self):
        ...
        # for i in genres: self.list.append({'name': cleangenre.lang(i[0], self.lang), 'url': self.genre_link % i[1], 'image': 'icon.png', 'action': 'tvshows'})
        # self.addDirectory(self.list)
        # return self.list
    def genres(self):
        try:
            ...
            for i in range(0, len(self.list)): self.list[i].update({'image': 'icon.png', 'action': 'tvshows'})
            self.addDirectory(self.list)
            return self.list
        except:
            return
    ...
    def networks(self):
        ...
        for i in networks: self.list.append({'name': i[0], 'url': self.network_link % (i[1]), 'image': 'icon.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i), 'url': self.certification_link % str(i).replace('-', '_').lower(), 'image': 'icon.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def tvshowDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                if not tvdb == "0" or tvdb == None: sysmetalliq = "plugin://plugin.video.metalliq/tv/add_to_library_parsed/%s/direct.bonecrusher.q" % tvdb
                else: sysmetalliq = "0"
