...
def viewClearCache(params):
    if cache.clearCacheFiles():
        ...
        dialog.notification('Odin Toons 5', 'Cache file cleared', xbmcgui.NOTIFICATION_INFO, 3500, True)
...
def viewClearTrakt(params):
    ...
    if trakt.clearTokens(ADDON):
        dialog = xbmcgui.Dialog()
        dialog.notification('Odin Toons 5', 'Trakt tokens cleared', xbmcgui.NOTIFICATION_INFO, 3500, False)
...
def viewResolve(params):
    ...
    if 'stream' in params:
        # BACKWARDS COMPATIBILITY: Odin Toons 5 0.4.4.
...
def logStreamError(api, showID, episodeID):
    dialog = xbmcgui.Dialog()
    dialog.notification('Odin Toons 5', 'No streams found', xbmcgui.NOTIFICATION_INFO, 3000, True)
    xbmc.log('Odin Toons 5 | No streams for API:%s SHOW:%s EPISODE:%s' % (api, showID, episodeID), xbmc.LOGWARNING)
