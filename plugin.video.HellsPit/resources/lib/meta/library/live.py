...
def setup_library(library_folder):
    if library_folder[-1] != "/":
        library_folder += "/"
    HellsPit_playlist_folder = "special://home/userdata/addon_data/plugin.video.HellsPit/playlists/mixed/HellsPit/"
    if not xbmcvfs.exists(HellsPit_playlist_folder): xbmcvfs.mkdir(HellsPit_playlist_folder)
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set HellsPit as a channel video source?")
        if dialogs.yesno("{0} {1}".format(_("Library"), "setup"), msg):
            source_thumbnail = get_icon_path("live")
            source_name = "HellsPit " + _("Channels")
