...
def tv_add_all_to_library(items, noscan = False):
    ...
    batch_add_file.close()
    xbmc.executebuiltin("RunPlugin(plugin://plugin.video.PortalGun/tv/batch_add_to_library)")
        ...
        for id in ids:
            ...
            if id == None or id == "None": pass
                ...
                try:
                    ...
                    xbmc.log("PortalGun failed to add: {0}".format(id),xbmc.LOGNOTICE)
            ids_index += 1
    ...
    if info['poster'] == None or info['poster'] == "":
        if imdb_id != None and imdb_id != "":
            ...
            if show != []:
                ...
                if info['fanart'] == None or info['fanart'] == "":
                    if show['backdrop_path'] != None and show['backdrop_path'] != "": info['fanart'] = u'%s%s' % ("http://image.tmdb.org/t/p/original", show['backdrop_path'])
    if info['poster'] == None or info['poster'] == "": info['poster'] = "https://github.com//bcrusher29/portal/raw/master/Style/unavailable.png"
    if info['fanart'] == None or info['fanart'] == "": info['fanart'] = get_background_path()
    if xbmc.getCondVisibility("system.hasaddon(script.qlickplay)"): context_menu = [(_("Scan item to library"),"RunPlugin({0})".format(plugin.url_for("tv_add_to_library", id=tvdb_id))), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.qlickplay,info=playtvtrailer,tvdb_id={0})".format(tvdb_id)), ("portalgun[COLOR ff64dd17]P[/COLOR]lay", "RunScript(script.qlickplay,info=tvinfo,tvdb_id={0})".format(tvdb_id)), ("%s %s (%s)" % ("Recommended", _("TV shows"), "TMDb"),"ActivateWindow(10025,plugin://script.qlickplay/?info=similartvshows&tvdb_id={0})".format(tvdb_id))]
    elif xbmc.getCondVisibility("system.hasaddon(script.interdimensional)"): context_menu = [(_("Scan item to library"),"RunPlugin({0})".format(plugin.url_for("tv_add_to_library", id=tvdb_id))), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.interdimensional,info=playtvtrailer,tvdb_id={0})".format(tvdb_id)), (_("Extended TV show info"), "RunScript(script.interdimensional,info=extendedtvinfo,tvdb_id={0})".format(tvdb_id)), ("%s %s (%s)" % ("Recommended", _("TV shows"), "TMDb"),"ActivateWindow(10025,plugin://script.interdimensional/?info=similartvshows&tvdb_id={0})".format(tvdb_id))]
...
def list_seasons_tvdb(id):
    ...
    for (season_num, season) in show.items():
        ...
        if xbmc.getCondVisibility("system.hasaddon(script.qlickplay)"): context_menu = [("portalgun[COLOR ff64dd17]P[/COLOR]lay", "RunScript(script.qlickplay,info=seasoninfo,tvshow={0},season={1})".format(title, season_num)), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.qlickplay,info=playtvtrailer,tvdb_id={0})".format(id)), (_("Recommended tv shows") + " (TMDb)","ActivateWindow(10025,plugin://script.qlickplay/?info=similartvshows&tvdb_id={0})".format(id))]
        elif xbmc.getCondVisibility("system.hasaddon(script.interdimensional)"): context_menu = [(_("Extended season info"), "RunScript(script.interdimensional,info=seasoninfo,tvshow={0},season={1})".format(title, season_num)), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.interdimensional,info=playtvtrailer,tvdb_id={0})".format(id)), (_("Recommended tv shows") + " (TMDb)","ActivateWindow(10025,plugin://script.interdimensional/?info=similartvshows&tvdb_id={0})".format(id))]
        else: context_menu = []
...
def list_episodes_tvdb(id, season_num):
    ...
    for (episode_num, episode) in season.items():
        ...
        elif xbmc.getCondVisibility("system.hasaddon(script.interdimensional)"): context_menu = [(_("Extended episode info"), "RunScript(script.interdimensional,info=episodeinfo,tvshow={0},season={1},episode={2})".format(title, season_num, episode_num)), ("%s %s" % (_("TV"), _("Trailer").lower()),"RunScript(script.interdimensional,info=playtvtrailer,tvdb_id={0})".format(id)), (_("Recommended tv shows") + " (TMDb)","ActivateWindow(10025,plugin://script.interdimensional/?info=similartvshows&tvdb_id={0})".format(id))]
        else: context_menu = []
