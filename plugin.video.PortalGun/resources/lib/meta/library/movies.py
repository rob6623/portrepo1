...
#    elif xbmc.getCondVisibility("system.hasaddon(script.interdimensional)"): xbmc.executebuiltin("RunScript(script.interdimensional,info=afteradd)")
...
def setup_library(library_folder):
    if library_folder[-1] != "/":
        library_folder += "/"
    PortalGun_playlist_folder = "special://home/userdata/addon_data/playlists/mixed/PortalGun/"
    if not xbmcvfs.exists(PortalGun_playlist_folder): xbmcvfs.mkdir(PortalGun_playlist_folder)
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set PortalGun as a movies video source?")
        if dialogs.yesno(_("Library setup"), msg):
            source_thumbnail = get_icon_path("movies")
            source_name = "PortalGun " + _("Movies")
