...
timeshift = xbmc.translatePath('special://profile/addon_data/script.GCguide/timezone.ini')
path_to_settings = xbmc.translatePath('special://profile/addon_data/script.GCguide')
current_db = xbmc.translatePath('special://profile/addon_data/script.GCguide/program.db')
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.GCguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.GCguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.GCguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        try:
            if os.path.exists(timeshift):
                os.remove(timeshift)
            xbmc.log('[script.GCguide] Updating caches...', xbmc.LOGDEBUG)
	...
	if not os.path.exists(xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator')):
            os.makedirs(xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator'))
        channelsList = self.getChannelList(False,True)
        channels = [channel.title for channel in channelsList]
        ini = xbmcvfs.File('special://profile/addon_data/plugin.video.Toxiccreator/custom_channels.ini','wb')
    ...
    def channelSetup(self, append=None):
        ...
        if xmltvType == '':
            xmltvType = 'Gas Chamber (Freeview UK)'
        elif xmltvType == 'Sub File':
            xmltvType = ADDON.getSetting('sub.xmltv')
        xmltvfile = xmltvType + '.ini'
        channelsList = self.getChannelList(False,True)
        ini = xbmcvfs.File('special://profile/addon_data/script.GCguide/resources/guide_setups/%s' % xmltvfile,'wb')
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide'))
    KEY = 'xmltv'
    LOGO_SOURCE_IVUE = 0
    LOGO_SOURCE_SUB = 1
...
#Karls custom xml import for creator
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            if addon.getSetting('custom.xmltv.type') == "0":
            	try:
                    ...
                    pass
                xmltvTemp = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'custom.xml'))
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[script.GCguide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback):
            ...
            if event == "end":
	            ...
	            if elem.findtext('title') is not None:
                    ...
                    if xmltvType == '':
                        ...
                        xmltvType = ADDON.getSetting('sub.xmltv')
                    xmltvfile = xmltvType + '.ini'
                    inifile = xbmc.translatePath('special://profile/addon_data/script.GCguide/resources/guide_setups/%s' % xmltvfile)
        ...
        #f = xbmcvfs.File('special://profile/addon_data/script.GCguide/program_category.ini',"wb")
    ...
    def parseXMLTVchannels(self, progress_callback):
        ...
        #dialog.create('Gas Chamber TV Guide', "Loading EPG Data")
        weight = -1
        chanList = []
