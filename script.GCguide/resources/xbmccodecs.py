...
def setSurfaceCodec():
    if surfaceCodec == False:
        ...
        utils.dialog.ok('Gas Chamber', "Media Codec (Surface) Enabled", "Please restart ivue for changes to take affect")
    else:
        xbmc.executeJSONRPC(jsonSetSurfaceCodec  % "false")
        utils.dialog.ok('Gas Chamber', "Media Codec (Surface) Disabled", "Please restart ivue for changes to take affect")

def setCodec():
    if codec == False:
        xbmc.executeJSONRPC(jsonSetCodec % "true")
        utils.dialog.ok('Gas Chamber', "Media Codec Enabled", "Please restart ivue for changes to take affect")
    else:
        xbmc.executeJSONRPC(jsonSetCodec  % "false")
        utils.dialog.ok('Gas Chamber', "Media Codec Disabled", "Please restart ivue for changes to take affect")
