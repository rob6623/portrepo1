...
try:
    ADDON = xbmcaddon.Addon('script.GCguide')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.GCguide)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.GCguide] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)