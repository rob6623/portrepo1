...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide'))
    filePath = ''
    fileUrl = ''
    addon = None
        ...
        if fetch:
            ...
            try:
                if self.fileUrl == utils.get_setting(addon_id,'xmltv.url') and self.fileUrl.endswith('.zip'):
                    dp = xbmcgui.DialogProgress()
                    zipPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'custom_tempfile.zip'))
                    zipurl = self.fileUrl
                    dp.create("Gas Chamber","Downloading guide data",'')
                    urllib.urlretrieve(zipurl,zipPath,lambda nb, bs, fs, url=zipurl: _pbhook(nb,bs,fs,zipurl,dp))
                    f.close()

                elif self.fileUrl.endswith('.zip'):
                    dp = xbmcgui.DialogProgress()
                    zipPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'tempfile.zip'))
                    zipurl = self.fileUrl
                    dp.create("Gas Chamber","Downloading guide data",'')
                    urllib.urlretrieve(zipurl,zipPath,lambda nb, bs, fs, url=zipurl: _pbhook(nb,bs,fs,zipurl,dp))
                    f.close()
            ...
            basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide'))
            zipPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'tempfile.zip'))
            customzipPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'custom_tempfile.zip'))
            tempzipPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'custom_tempfile'))
            customPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'custom.xml'))
            if os.path.exists(zipPath):
                ...
                xbmc.log('[script.GCguide] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
