...
ADDON = xbmcaddon.Addon(id='script.GCguide')
...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'guides.ini'))
    ...
    def __init__(self):
        try:
            ...
            xbmc.log('[script.GCguide] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[script.GCguide] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[script.GCguide] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[script.GCguide] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[script.GCguide] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[script.GCguide] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.mekguide', 'settings.xml'))
            ...
            if not ADDON.getSetting('categories.path') == 'Gas Chamber':
                yes_pressed = d.yesno('[COLOR ffff7e14][B]Gas Chamber SETUP[/B][/COLOR]', 'Do you wish to use Gas Chamber categories',nolabel='no',yeslabel='yes')
                if yes_pressed:
                    ADDON.setSetting('categories.path', 'Gas Chamber')
        
            if not int(ADDON.getSetting('logos.source')) == 0:
                yes_pressed = d.yesno('[COLOR ffff7e14][B]Gas Chamber SETUP[/B][/COLOR]', 'Do you wish to use Gas Chamber logo pack',nolabel='no',yeslabel='yes')
                if yes_pressed:
