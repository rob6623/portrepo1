...
path_to_channels = xbmc.translatePath('special://profile/addon_data/script.GCguide/%s' % typeName)
...
def Timeshift():
    ...
    d.ok('Gas Chamber TV Guide', 'Your selected channels have been shifted.', 'They will be re-configured next time', 'you launch the guide')
    c.close()
...
def resetTimeshift():
    ...
    if os.path.exists(timeshift):
        ...
        d.ok('Gas Chamber Time shift', 'No channels have been found','')
        return
    if len(saved) <= 0:
        d.ok('Gas Chamber Time shift', 'No channels have been found','')
        return
    ...
    if not selections:
        d.ok('Gas Chamber Time shift', 'No channels have been reset','')
        return
    else:
        for selection in selections:
        ...
        for item in saved:
            if len(item) > 1:
                f.writelines('%s' % item)
        f.close()
        d.ok('Gas Chamber TV Guide', 'Your selected channels have been reset.', 'They will be re-configured next time', 'you launch the guide')
        c.close()
