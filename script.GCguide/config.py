...
#Karls changes
skins1 = ('http://reaperslockedmadhouse.com/gcguide')
ivue = utils.folder()
d = xbmcgui.Dialog()
dp = xbmcgui.DialogProgress()
skin = addon_name.getSetting('skin')
path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'skins'))
Skinxml = xbmc.translatePath(os.path.join(path, skin, '720p', 'script-tvguide-main.xml'))
xmlData = xbmc.translatePath('special://profile/addon_data/script.GCguide/resources/config/Data.txt')
logoData = xbmc.translatePath('special://profile/addon_data/script.GCguide/resources/config/Logo.txt')
catData = xbmc.translatePath('special://profile/addon_data/script.GCguide/resources/categories/')	
ivue = utils.folder()
...
def listskins():
    ...
    if os.path.exists(path):
        for name in os.listdir(path): 
            files.append(name)
        skin = d.select("Gas Chamber", files)
        if skin == -1:
            return
        else:
            selected = files[skin]
        ...
        d.ok('Gas Chamber', 'Skin Folder Missing', '',"Please run Gas Chamber to complete setup")

def getskins():
    folder = skins1+'/skins/'
    view = openURL(folder)
    match=re.compile('<a href="(.*?)">').findall(view)
    notneeded = ['/ivueguide/', '/gcguide/', 'index.htmlofflineforabit', '/ivueguide//','/ivueguidexml//', 'skins.zip']
    files = [] 
    for name in match:
    ...
    if skin == -1:
        ...
        dp.create("Gas Chamber","Downloading %s" % selected,'')
        ...
        if os.path.exists(path + "/%s" % selected): 
            ...
            if selected == 'Transparent Vue':
                ...
                addon_name.setSetting('transparent.enabled', 'false')
            d.ok('Gas Chamber', 'Download complete', '', '%s is now set as current skin' % selected)
        else:
            d.ok('Gas Chamber', 'Download failed', 'Please try downloading again')
...
def delskins():
    ...
    if os.path.exists(path):
        ...
        if skin == -1:
            ...
            selected = files[skin]
            installed = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'skins', '%s' % selected))
            shutil.rmtree(installed)
            if not os.path.exists(installed):
                d.ok("Gas Chamber", '','%s has been successfully removed' % selected, " [COLOR gold]Brought To You By Gas Chamber[/COLOR]")
            else:
                d.ok("Gas Chamber", '','%s was not removed' % selected, " [COLOR gold]Please try again[/COLOR]")
    else:
        d.ok('Gas Chamber', 'No skins installed', '',"Please run Gas Chamber to complete setup")
...
def customiseColours(texture):
        ...
        if texture == 'backgroundTexture':
            ...
            colourDialog = d.browseSingle(type=2, heading=header, shares='files', mask='.png', useThumbs=True, treatAsFolder=True, defaultt='special://profile/addon_data/script.GCguide/resources/skins/Transparent Vue/media/colors/')
            if colourDialog:
                try:
            ...
            colourDialog = d.browse(type=2, heading=header, shares='files', mask='.png', useThumbs=True, treatAsFolder=False, defaultt='special://profile/addon_data/script.GCguide/resources/skins/Transparent Vue/media/colors/')
            if colourDialog > 0:
                colour = str(colourDialog).split('colors/')[1].split('.png')[0]
        ...
        if addon_name.getSetting('skin') == 'Transparent Vue':
            ...
            for file in dirs:
                ...
                if match:
                    ...
                    f2.write(m)       
                    f2.close()
        d.ok("Gas Chamber", '',' SUCCESSFUL :)', " [COLOR gold]Brought To You By Gas Chamber[/COLOR]")
...
def updater():
	...
	d.ok("Gas Chamber", '',' CHECKING FOR REPO UPDATES SUCCESSFUL :)', " [COLOR gold]Brought To You By Gas Chamber[/COLOR]")
	return
	...
	for name, value in matches:
        ...
        if selection < 0:
            ...
            if os.path.exists(os.path.join(catData,check_files+'.ini')) and not addon_name.getSetting('categories.path') == check_files:
                yes_pressed = d.yesno('[COLOR ffff7e14][B]Gas Chamber SETUP[/B][/COLOR]', 'Do you wish to use ' +check_files+ ' categories',nolabel='no',yeslabel='yes')
                if yes_pressed:
	        ...
	        for name, value in matches:
                    if name == check_files:
                        yes_pressed = d.yesno('[COLOR ffff7e14][B]Gas Chamber SETUP[/B][/COLOR]', 'Do you wish to use ' +check_files+ ' logo pack',nolabel='no',yeslabel='yes')
                        if yes_pressed:
                            addon_name.setSetting('logos.source', '1')
                            addon_name.setSetting('sub.logos', check_files)
                            addon_name.setSetting('sub.logos.url', value)
            d.ok('Gas Chamber', 'Please press ok in settings dialog to confirm changes, then reload guide for changes to take affect', '')
...
def scrollText():
    scrolling = d.select("Gas Chamber", ['Disable scrolling', 'Enable scrolling'])
    if scrolling == 0:
...
def fontSize():
    font = d.select("Gas Chamber", ['Enable large font', 'Disable large font'])
    if font == 0:
        if os.path.exists(Skinxml):
            f1=open(Skinxml,'r').read()
            if 'font' in f1:
