...
class TVGuide(xbmcgui.WindowXML):
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() == ACTION_LEFT:
            ...
            xbmc.log('[script.GCguide] Unhandled ActionId: '
            ...
            if program.startDate and program.endDate:
                if program.endDate < datetime.datetime.now():
                    ...
                    if program.notificationScheduled:
                        yes_pressed=utils.dialog.yesno('[COLOR ffff7e14][B]Gas Chamber REMINDER[/B][/COLOR]', 'Remove reminder for ', '[COLOR ffff7e14]'+program.title+'[/COLOR]  '+self.formatTime(program.startDate),nolabel='Cancel',yeslabel='Remove')
                        ...
                        yes_pressed=utils.dialog.yesno('[COLOR ffff7e14][B]Gas Chamber REMINDER[/B][/COLOR]', 'Set reminder for ', '[COLOR ffff7e14]'+program.title+'[/COLOR]  '+self.formatTime(program.startDate),nolabel='Cancel',yeslabel='Set')
    ...
    def _showSchMenu(self, program):
        ...
        resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Schedule[/COLOR]', ['Today (%s)' % (now), '%s' % (one), '%s' % (two)])
        if resp <0:
    ...
    def _showToolsMenu(self, program):
        ...
        if xbmc.getCondVisibility('system.platform.android'):
            ...
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Tools[/COLOR]', ['Gas Chamber Creator', 'View Kodi Log', 'Purge Database', 'Soft Reset', 'Hard Reset', xbmccodecs.media, xbmccodecs.surfacemedia]) 
        else:
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Tools[/COLOR]', ['Gas Chamber Creator', 'View Kodi Log', 'Purge Database', 'Soft Reset', 'Hard Reset'])

        if resp <0:
            self._skinexitMenu(program)            
        if resp == 0:
			    xbmc.executebuiltin('RunAddon(plugin.video.Toxiccreator)')        
        if resp == 1:
			    import log_viewer  
        if resp == 2:
            self.close()
    ...
    def programSearch(self):
        action = utils.dialog.select("Gas Chamber Search", ['Search by program', 'Search by channel'])
        if action == -1:
            return
        if action == 0:
            self.searchProg()
    ...
    def searchProg(self, search=None):
        ...
        try:
            ...
            title = ''
        file_name = "special://profile/addon_data/script.GCguide/title_search.list"
    ...
    def getReminders(self):
        ...
        if buttonClicked == 1003:
            ...
            yes=utils.dialog.yesno("Gas Chamber Planner", 'You are about to delete all saved programs', 'Are you sure?', nolabel='No', yeslabel='Yes');
            if yes:	
...
class PopupMenu(xbmcgui.WindowXMLDialog):
    ...
    C_POPUP_ADDON_IMAGE_iVue = 'special://home/addons/script.GCguide/resources/png/shortcut.png'


    settings = utils.xbmcvfs.File('special://home/addons/script.GCguide/resources/settings.xml','rb').read()
    ...
    actions = ["[COLOR red]Reset current shortcut[/COLOR]","[COLOR yellow]Gas Chamber default shortcuts[/COLOR]"] + names
...
........"""
    ...
    def onInit(self):
        ...
        for x in range(1, 11):
            ...
            if ADDON.getSetting('CustomAddon%s.label' % x) == '':
                self.getControl(TITLE).setLabel('Gas Chamber Shortcut %s' % x)             
            else:
    ...
    def _editShortcutMenu(self):
        resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', ['Edit Shortcuts', 'Use Latest Gas Chamber Selection'])
        if resp < 0:
            return
        if resp == 0:
    ...
    def _showShortcutMenu(self):
        ...
        for x in range(1, 11):
            if  not ADDON.getSetting('CustomAddon%s.label' % x) == '':
                ...
                shortcut = 'Gas Chamber Shortcut %s' % x
                shortcuts.append(shortcut)
        if SKIN == 'sky Vue' or SKIN == 'Gas Chamber Default':
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', shortcuts[:9])
        elif SKIN == 'iVurgin':
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', shortcuts[:5])
        elif SKIN == 'FreeVue Play' or SKIN == 'Gas Chamber Classic':
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', shortcuts[:4])
        elif SKIN == 'Sky Classic':
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', shortcuts[:8])
        else:
            resp = utils.dialog.select('[COLOR fffea800]Gas Chamber Shortcuts Menu[/COLOR]', shortcuts)
        if resp < 0:
            return
        if resp == 0:
            self.shortcut(1)
    ...
    def shortcut(self, shortcut):
       ...
       if fav == -1:
           ...
           self.getControl(TITLE).setLabel('Gas Chamber Shortcut%s' % shortcut)
...
........"""
    ...
    def onAction(self, action):
        if action.getId() in [KEY_CONTEXT_MENU]:
            ...
            if kodi < 16:
                utils.dialog.ok('Gas Chamber', 'Editing categories in Kodi %s is currently not supported.' % kodi, 'If you would like to use this option please update kodi to the latest stable version or you can import you own catergories.ini via the settings. Thank you for using Gas Chamber')
            else:
    ...
    def onClick(self, controlId):
        if controlId == self.C_CAT_CATEGORY:
            ...
            kodi = float(xbmc.getInfoLabel("System.BuildVersion")[:4])
            if kodi < 16:
                utils.dialog.ok(' Gas Chamber', 'Adding categories in Kodi %s is currently not supported.' % kodi, 'If you would like to use this option please update kodi to the latest stable version or you can import you own catergories.ini via the settings. Thank you for using Gas Chamber')
            else:
...
class los_List(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        if found == 0:
            utils.dialog.ok('[COLOR fffea800]Gas Chamber[/COLOR]', 'No fixtures found','') 
            self.close()
    ...
    def getContent(self, url):
        try:
            ...
            xbmc.executebuiltin("Dialog.Close(busydialog)")
            utils.dialog.ok('[COLOR fffea800]Gas Chamber[/COLOR]', 'Problem finding fixtures','Please try again soon')
            self.close() 
            ...
            if item:
                ...
                if channels >=0:
                    ...
                    if result:
                        ...
                        xbmc.executebuiltin("Dialog.Close(busydialog)")
                        utils.dialog.ok('[COLOR fffea800]Gas Chamber[/COLOR]', 'No streams found please try another channel '+ mychannels[channels],'') 
