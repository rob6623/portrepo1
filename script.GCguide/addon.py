...
FORCE       = xbmc.translatePath(os.path.join('special://home', 'addons', 'script.GCguide', 'force.py'))
ivue = utils.folder()
skin = ADDON.getSetting('skin')
default = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'skins', 'Default'))
setSkin = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'skins', skin))
cat1 = ('http://reaperslockedmadhouse.com/gcguide')
SkinFolder = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'skins'))
iniPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'addons2.ini')) 
iniFile = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'addons.ini'))
catchupFile = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'catchup.xml'))
catPath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.GCguide', 'resources', 'categories'))
ivuecatsFile = xbmc.translatePath(os.path.join(catPath,'GC.ini'))
customcatsFile = xbmc.translatePath(os.path.join(catPath,'custom.ini'))
addons_index_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/addons_index.ini')
ivueFile = cat1+'/categories.ini'
    ...
    if os.path.exists(iniPath) and os.path.exists(addons_index_path) and not interval == INTERVAL_OFF:
        if interval <> INTERVAL_ALWAYS:
            ...
            if ((interval == INTERVAL_12 and diff >= 43200) or
                ...
                xbmc.executebuiltin('RunPlugin(plugin://plugin.video.Toxiccreator/update)')
            else:
                w = gui.TVGuide()
                w.doModal()
                del w
...
try:
    if os.path.exists(FORCE) and os.path.exists(SkinFolder):
        ...
        dp.create("Gas Chamber","Downloading latest Skin Pack",'')
    ...
    if not os.path.exists(default):
        ...
        dp.create("Gas Chamber","Downloading Default Skin",'')
        urllib.urlretrieve(url,zipfile,lambda nb, bs, fs, url=url: config._pbhook(nb,bs,fs,url,dp))
        config.extract(zipfile, SkinFolder)
        ADDON.setSetting('scroll.chan', 'Enabled')
        ADDON.setSetting('font', 'Disabled')
    ...
    if not os.path.exists(setSkin):
        try:
            ...
            dp.create("Gas Chamber","Downloading %s" % skin,'')
            urllib.urlretrieve(url,zipfile,lambda nb, bs, fs, url=url: config._pbhook(nb,bs,fs,url,dp))
            config.extract(zipfile, SkinFolder)
            ADDON.setSetting('scroll.chan', 'Enabled')
            ADDON.setSetting('font', 'Disabled')
    ...
    if ADDON.getSetting('ivue.addons.ini') == 'true':
        dp.create("Gas Chamber","Downloading files",'')
        try:
    ...
    if not os.path.exists(iniFile):
        dp.create("Gas Chamber","Downloading addons.ini",'')
        urllib.urlretrieve(ivueINI,iniFile,lambda nb, bs, fs, url=ivueINI: config._pbhook(nb,bs,fs,url,dp))
        dp.close()


    if not os.path.exists(catchupFile):
        dp.create("Gas Chamber","Downloading catchup.xml",'')
        urllib.urlretrieve(ivueCatchup,catchupFile,lambda nb, bs, fs, url=ivueCatchup: config._pbhook(nb,bs,fs,url,dp))
        dp.close()


    if not os.path.exists(ivuecatsFile):
        dp.create("Gas Chamber","Downloading categories file",'')
        urllib.urlretrieve(ivueFile,ivuecatsFile,lambda nb, bs, fs, url=ivueFile: config._pbhook(nb,bs,fs,url,dp))
        dp.close()

except:
    d.ok('Gas Chamber Server Notice', 'Looks like we Couldnt connect to the Gas Chamber server.', 'Please check your internet connection (Gas Chamber maybe offline).', 'Guide may not work properly in the mean time')
...
try:
    ...
    d.ok('Gas Chamber Notice', 'Looks like we Couldnt retrieve your custom categories file', 'Please check your file path or internet connection if using url', 'Gas Chamber may not work properly in the mean time')
    pass
...
# set filepath
addons_data_ini_path = xbmc.translatePath('special://profile/addon_data/plugin.video.Toxiccreator/addons_index.ini')
old_ini_path = xbmc.translatePath('special://home/addons/plugin.video.Toxiccreator/addons_index.ini')
