                ...
                if self.type == "tv" or self.type == "episode":
                    if self.listitem.getProperty("dbid"):
                        ...
                        if os.path.exists(show_playlist):
                            url = show_playlist
                        else: url = 'plugin://plugin.video.HellsPit/tv/play/%s/1/1/%s' % (tvdb_id, SETTING("player_alt"))
                    else: url = 'plugin://plugin.video.HellsPit/tv/play/%s/1/1/%s' % (tvdb_id, SETTING("player_alt"))
                else:
                    if self.listitem.getProperty("dbid"): url = 'temp'
                    else: url = 'plugin://plugin.video.HellsPit/movies/play/tmdb/%s/%s' % (item_id, SETTING("player_alt"))
            ...
            if selection == 1:
                if self.type == "tv" or self.type == "episode":
                    ...
                    if self.listitem.getProperty("dbid"):
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.HellsPit/tv/add_to_library/%s)" % tvdb_id)
                    ...
                    if self.listitem.getProperty("dbid"):
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.HellsPit/movies/add_to_library/tmdb/%s)" % item_id)
                ...
                self.getControl(500).selectItem(self.position)
            if selection == 2:
                if self.type == "tv": url = "plugin://script.SKI/?info=playtvtrailer&id=%s" % item_id
                elif self.type == "episode": url = "plugin://script.SKI/?info=playtvtrailer&tvdb_id=%s" % tvdb_id
                elif self.type == "movie": url = "plugin://script.SKI/?info=playtrailer&id=%s" % item_id
            ...
            if selection == 3:
                if self.logged_in:
                    ...
                    if self.type == "episode":
                        path = xbmc.translatePath("special://profile/addon_data/script.SKI/exclusions_tvdb.txt")
